    ...
    <div class="bg-gray-800 border border-gray-700 rounded-2xl p-8">
        <form method="POST" action="{{ route('flights.update', $flight->id_flights) }}" class="space-y-6">
            ...
            <div>
                ...
                <p class="text-gray-500 text-xs mb-3">Seleccioná los miembros de tripulación para este vuelo</p>
                    ...
                    @forelse($crews as $crew)
                        <label class="flex items-center gap-3 cursor-pointer hover:bg-gray-600 px-2 py-1.5 rounded-lg transition">
                            <input type="checkbox" name="crew_members[]" value="{{ $crew->id_crew_member }}"
                                {{ in_array($crew->id_crew_member, old('crew_members', $assignedCrew)) ? 'checked' : '' }}
