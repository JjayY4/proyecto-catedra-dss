...
<body class="bg-gray-900 text-white min-h-screen">
    ...
    <div class="max-w-2xl mx-auto px-6 py-10">
        ...
        <div class="mb-8">
            <a href="{{ route('routes.index') }}" class="text-gray-400 hover:text-white text-sm flex items-center gap-1 mb-4 transition">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/>
                </svg>
                Volver a rutas
            </a>
            <h1 class="text-3xl font-bold text-white">Registrar Ruta</h1>
    ...
    <form method="POST" action="{{ route('routes.store') }}" class="space-y-6">
        ...
        <div>
            <label class="block text-sm font-medium text-gray-300 mb-1.5">Aeropuerto de Origen</label>
        ...
        <div>
            <label class="block text-sm font-medium text-gray-300 mb-1.5">Ciudad de Origen</label>
        ...
        <div>
            <label class="block text-sm font-medium text-gray-300 mb-1.5">Aeropuerto de Destino</label>
        ...
        <div>
            <label class="block text-sm font-medium text-gray-300 mb-1.5">Ciudad de Destino</label>
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                <label class="block text-sm font-medium text-gray-300 mb-1.5">Distancia Km</label>
                <input type="number" name="distance_km" value="{{ old('distance_km') }}" min="1" step="0.01" placeholder="Ej: 1650" required
                    class="w-full bg-gray-700 border border-gray-600 text-white rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent placeholder-gray-400">
                @error('distance_km')
                    <p class="text-red-400 text-xs mt-1">{{ $message }}</p>
                @enderror
